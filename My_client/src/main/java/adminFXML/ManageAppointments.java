...
public class ManageAppointments implements Initializable{
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (appointments.size() != 0)
            for (Appointment appointment : appointments) {
                AppointmentModel tableAppointment = new AppointmentModel(appointment.getId(), appointment.getStarTime().toString(), appointment.getDate().toString(), appointment.getService().getService().getName(), appointment.getClient().getName());
    ...
    public void onCheck() {
        ...
        try(FileWriter writer = new FileWriter("check.txt", true))
        {
            ...
            String time = "Время получения: ";
            writer.append("***************************************************");
            writer.append('\n');
            writer.write(number + appointmentModel.getService().toString());
            writer.append('\n');
            writer.write(client + appointmentModel.getClient().toString());
            writer.append('\n');
            writer.write(dat + appointmentModel.getDate().toString());
            writer.append('\n');
            writer.write(time + appointmentModel.getStartTime().toString());
            writer.append('\n');
            writer.append('\n');
            writer.append('\n');
